...
def js_text(data):
    # Compact like the other generated data files; the JSON asset stays readable.
    value = json.dumps(data, ensure_ascii=False, separators=(',', ':'), allow_nan=False).replace('&', '\\u0026').replace('<', '\\u003c').replace('>', '\\u003e').replace('\u2028', '\\u2028').replace('\u2029', '\\u2029')
