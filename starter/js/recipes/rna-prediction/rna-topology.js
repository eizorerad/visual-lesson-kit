...
R.register({
 ...
 chapter:['Псевдоузлы и границы моделей','Pseudoknots and model limits'],
 source:['Lyngsø & Pedersen, 2000 · Pseudoknot complexity','https://doi.org/10.1089/106652700750050862'],
 states:[
  ['Начнём с учебной последовательности и двух вложенных пар.','Start with a toy sequence and two nested pairs.',
   'Новый учебный пример: последовательность GCAAGCAAGCAAGC и заданные пары (1,10), (2,9). Дуги связывают индексы партнёров, круглые скобки кодируют пары, точки — непарные позиции. Каждая пара записана в матрице дважды, симметрично относительно диагонали. Цвет обозначает набор пар, а не тип основания. Схема намеренно мала и не утверждает, что эта последовательность реально образует показанный псевдоузел.',
   'This new toy example has sequence GCAAGCAAGCAAGC and supplied pairs (1,10), (2,9). Arcs link partner indices; parentheses encode pairs and dots mark unpaired positions. Each pair is recorded twice in the matrix, symmetrically across the diagonal. Color identifies the pair set, not the base type. This deliberately small schematic does not claim that the sequence actually forms the displayed pseudoknot.'],
  ...
  ['Матрица сохраняет все четыре пары и свою симметрию.','The matrix retains all four pairs and remains symmetric.',
   'В матрице появились записи (5,14), (14,5), (6,13), (13,6). Псевдоузел можно записать матрицей пар; каждая позиция в этом примере по-прежнему имеет не больше одного партнёра. Запись ещё не доказывает физическую допустимость структуры. В сравнительном анализе значимая ковариация сверх ожидаемой от общего происхождения может поддерживать пары-кандидаты, в том числе пересекающиеся по индексам. Но обнаружение требует информативного выравнивания и достаточной изменчивости: оно не гарантирует нахождение всех псевдоузлов и не является общим детектором любых 3D-контактов. Эти четыре пары заданы автором, их ковариация не проверялась. <a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC7657543/" target="_blank" rel="noopener">Rivas, 2020.</a>',
   'The matrix gains entries (5,14), (14,5), (6,13), and (13,6). A pair matrix can record a pseudoknot; each position in this example still has at most one partner. The record alone does not establish physical validity. In comparative analysis, covariation significant above shared-ancestry expectation can support candidate pairs, including pairs that cross in index order. Detection requires an informative alignment and sufficient variation: it does not guarantee recovery of every pseudoknot and is not a general detector of arbitrary 3D contacts. These four pairs are supplied by the author; no covariation test was run on them. <a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC7657543/" target="_blank" rel="noopener">Rivas, 2020.</a>'],
  ['Пересечение дуг не означает, что цепь проходит сквозь себя.','Crossing arcs do not mean that the chain passes through itself.',
   'Для пар (i,j) = (1,10) и (k,l) = (5,14) выполняется i < k < j < l: 1 < 5 < 10 < 14. Это пересечение на схеме индексов, а не пересечение ковалентного остова в пространстве. Стандартное динамическое программирование по вложенным интервалам исключает такую связь между внутренней и внешней частями интервала. Специальные полиномиальные алгоритмы допускают ограниченные классы псевдоузлов; например, алгоритм Rivas–Eddy требует O(n⁶) времени и O(n⁴) памяти. Для широкого класса энергетических моделей поиск оптимума с произвольными псевдоузлами NP-труден; соответствующая задача решения NP-полна. Это утверждение о классе задачи, а не запрет на решение отдельных случаев. Дуги не показывают путь фолдинга. <a href="https://pubmed.ncbi.nlm.nih.gov/9925784/" target="_blank" rel="noopener">Rivas & Eddy, 1999</a>; <a href="https://doi.org/10.1089/106652700750050862" target="_blank" rel="noopener">Lyngsø & Pedersen, 2000.</a>',
   'For pairs (i,j) = (1,10) and (k,l) = (5,14), i < k < j < l holds: 1 < 5 < 10 < 14. This is a crossing in an index diagram, not an intersection of the covalent backbone in space. Standard dynamic programming over nested intervals excludes this link between the inside and outside of an interval. Specialized polynomial algorithms admit restricted pseudoknot classes; for example, Rivas–Eddy uses O(n⁶) time and O(n⁴) memory. For a broad class of energy models, optimization with arbitrary pseudoknots is NP-hard; the corresponding decision problem is NP-complete. This describes a problem class and does not prevent solving individual instances. Arcs do not show a folding pathway. <a href="https://pubmed.ncbi.nlm.nih.gov/9925784/" target="_blank" rel="noopener">Rivas & Eddy, 1999</a>; <a href="https://doi.org/10.1089/106652700750050862" target="_blank" rel="noopener">Lyngsø & Pedersen, 2000.</a>']
 ],
 qa:[
  ...
  {q:['Можно ли использовать динамическое программирование для псевдоузлов?','Can dynamic programming be used for pseudoknots?'],a:['Да. Для ограниченных классов существуют полиномиальные алгоритмы, хотя стандартная вложенная интервальная декомпозиция пересекающиеся пары исключает. Для общего поиска с произвольными псевдоузлами многие энергетические формулировки NP-трудны.','Yes. Restricted classes admit polynomial algorithms, although standard nested interval decomposition excludes crossing pairs. Many energy formulations of general optimization with arbitrary pseudoknots are NP-hard.'],source:'Lyngsø & Pedersen, 2000',url:'https://doi.org/10.1089/106652700750050862'},
  {q:['Гарантирует ли ковариация нахождение всех псевдоузлов?','Does covariation guarantee recovery of every pseudoknot?'],a:['Нет. Значимая ковариация может поддержать пересекающиеся пары-кандидаты, но её обнаружение зависит от выравнивания и изменчивости. Она не выявляет произвольные 3D-контакты и не гарантирует полную структуру.','No. Significant covariation can support crossing candidate pairs, but detection depends on alignment quality and variation. It does not identify arbitrary 3D contacts or guarantee a complete structure.'],source:'Rivas, 2020 · CaCoFold',url:'https://pmc.ncbi.nlm.nih.gov/articles/PMC7657543/'}
 ],
 build(ctx,v){
  const q=F.group(v.svg),state={second:0,map:0,witness:0};
  R.box(q,70,147,730,42,'Два набора пар','Two pair sets',27);
  R.box(q,847,147,358,56,'Симметричная карта пар','Symmetric pair map',26);
