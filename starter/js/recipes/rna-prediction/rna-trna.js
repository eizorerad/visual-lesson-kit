...
R.register({
 id:'rna-trna-real',chapter:['Что нужно предсказать?','What must be predicted?'],
 title:['Что нужно предсказать для одной РНК?','What must we predict for one RNA?'],
 status:['Известный пример · 2D и 3D структуры 1EHZ','Known reference · 2D and 3D of 1EHZ'],
 source:['Shi & Moore, 2000 · PDB 1EHZ','https://www.rcsb.org/structure/1EHZ'],
 states:[
  ['Начнём с известной тРНК: какие пары и какую 3D-форму нужно восстановить?',
   'Start with a known tRNA: which pairs and 3D shape would a predictor need to recover?',
   'Здесь одна дрожжевая тРНК(Phe), цепь A структуры 1EHZ: известный экспериментальный пример для постановки задачи, а не результат запущенного предиктора. Все 76 нуклеотидов изображены в авторской плоской схеме: короткие поперечные линии показывают пары четырёх стеблей. Расстояния и углы этой 2D-схемы условны. Номера остатков сохраняются при смене представления; модифицированные нуклеотиды не заменены немодифицированными. Полные идентификаторы компонентов сохранены в данных и подсказках точек.',
   'This is one yeast tRNA(Phe), chain A of structure 1EHZ: a known experimental reference that defines the task, not the output of a predictor run. All 76 residues appear in an authored planar diagram; short cross-lines show base pairs in the four stems. Distances and angles in this 2D diagram are schematic. Residue numbers persist when the representation changes; modified nucleotides are not replaced with unmodified ones. Full component IDs are retained in the data and point tooltips.'],
 ...
 qa:[
  ...
  {q:['Это предсказание структуры или траектории фолдинга?','Is this a structure or folding-pathway prediction?'],a:['Нет. Показан известный экспериментальный пример 1EHZ. 2D и 3D сопоставлены авторским движением; заключительный поворот меняет только ракурс.','No. This is the known experimental reference 1EHZ. Authored motion compares the 2D and 3D representations; the final rotation changes only the view.']}
 ],
 build(ctx,v){
