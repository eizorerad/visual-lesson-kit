...
R.register({id:'rna-geometry',title:['Одни пары — разные расположения в 3D','The same pairs, different 3D arrangements'],
 ...
 states:[
 ['Карта пар хранит партнёров, но не угол между двумя стеблями.','A pair map stores partners, but not the angle between two stems.','После схем индексов вернёмся к пространству. В новом учебном примере две шпильки GGACGAAACGUCC соединены тремя A: всего 29 позиций и 10 заданных пар. Цвета обозначают два стебля. Слева одна неизменная карта пар, справа её условная пространственная реализация. Размеры и углы не взяты из экспериментальной структуры.','After the index diagrams, return to space. In this new toy example two GGACGAAACGUCC hairpins are joined by three A residues: 29 positions and 10 supplied pairs in total. Colors identify the two stems. A fixed pair map is on the left; a schematic spatial realization is on the right. Dimensions and angles do not come from an experimental structure.'],
 ...
 ['Дальний контакт добавляет условие: два участка должны встретиться.','A long-range contact adds a condition: two regions must meet.','Золотая скобка выделяет сближенные петли как условный пример дополнительного пространственного ограничения. Она не означает новую каноническую пару или конкретную водородную связь. Именно такие дополнительные ограничения помогают выбрать пространственное расположение. Далее увидим реальную тРНК и конкретные контакты по экспериментальным координатам.','The gold bracket marks nearby loops as a schematic additional spatial constraint. It does not represent a new canonical pair or a specific hydrogen bond. Such additional constraints help select a spatial arrangement. Next we examine actual tRNA and specific contacts using experimental coordinates.']
