...
R.finish=function(){
 const order=['rna-map','rna-protein','rna-nucleotide','rna-chemistry','rna-pairs','rna-loops','rna-representations','rna-pseudoknot','rna-geometry','rna-trna-real','rna-docking','rna-anchors','rna-ions','rna-cotranscription','rna-evidence','rna-ml','rna-takeaways'];
 const chapters=[['Основания и цепь','Bases and backbone'],['Пары и форма','Pairs and shape'],['Контакты и среда','Contacts and environment'],['Путь и предсказание','Pathway and prediction']];
 episodes.sort((a,b)=>order.indexOf(a.id)-order.indexOf(b.id)).forEach(def=>{
  const i=order.indexOf(def.id);if(i<0)throw Error('Missing teaching order: '+def.id);
  def.chapter=chapters[i<6?0:i<10?1:i<13?2:3];R.mount(def);
 });
};
