...
- Choose the viewer's question, visible operation, result and next question. Preserve the identity of the same scientific object. Distinguish records, copies, aggregates, estimates and model expectations. Keep units and comparable scales explicit. Invented examples are labelled; paper claims retain their sources.
- Compose new SVG views and local helpers when useful. A catalog entry is optional; a missing component does not forbid new work. Use verified numerical models independently of their ready-made views. A schematic motion is not a computed biological trajectory.
- Keep the 1280×720 stage and compact five-control shell. Drawing area: x60–1220/y147–610. Use live semantic C colors, selectable bundled fonts and complete font licenses. Preserve the minimal canvas, notes, QA and sources. Look up `theme` or `shell` when changing them.
- Use `L.textBox` or `L.contract` for intended text regions; allocate real space instead of silently shrinking or clipping. Use the `clearance` card when labels meet lines, contours or moving geometry. Text containment alone does not prove visual clearance.
