...
- [Spatial biology](spatial-biology.html): two complete RU/EN 3D scenes (16 states), from cell labelling through bead/droplet capture to cell barcode, feature identity and UMI. [Guide](../docs/three-dimensional.md), reusable components `starter/js/three/`, editable recipes `starter/js/recipes/spatial-biology/`, build `python3 tools/build-spatial-biology.py` and verify with `--check`. Scaffold with `--template spatial-biology`. Procedural geometry and motion are illustrative; co-capture precedes synthesis in the depicted original solid-bead Drop-seq CITE-seq workflow.
