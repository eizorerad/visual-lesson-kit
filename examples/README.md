...
- [ATAC-seq film](atac-seq.html): 46 cues / 240.6 seconds, source-backed chromatin/Tn5 and synthetic readout. [Repeat/remix guide](../docs/atac-seq.md), edit `starter/js/atac-config.js`, scaffold `--template atac-seq`.
- [ATAC components](atac-components.html): four independent scenes / 16 states with source structures, fragment-origin geometry, readout and histogram. [Assembly guide](../docs/atac-components.md), edit `starter/js/recipes/atac/components.js`, scaffold `--template atac-components`. Both ATAC examples build with `python3 tools/build-atac.py`; add `--check` for fresh-build comparison.
- [tRNA journey](trna-journey.html): continuous 6:38 / 39-cue sequence-to-atoms film, including D/T contacts, hydrated Mg and atom-sphere packing. [Repeat/remix guide](../docs/trna-journey.md), editable `starter/js/trna-config.js`, build `python3 tools/build-trna-journey.py` and compare with `--check`. Scaffold with `--template trna-journey --palette ocean`.
...
- [RNA folding](rna-folding.html): 17 scenes / 63 authored states, RU/EN. [Guide](../docs/rna-folding.md), source `starter/js/recipes/rna-folding/`, build `python3 tools/build-rna-folding.py`.
