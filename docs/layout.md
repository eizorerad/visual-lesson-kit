...
Always give `setText` the full source-language string. The primitive translates that full string with `D.i18n.text` before splitting lines, and excludes its generated descendants from the general translator. Translating separate `tspan` fragments would lose the exact full-sentence dictionary key. Construction, source-text changes, and watched language changes publish the current localized string synchronously, including while detached; unavailable geometry remains unmeasured. A mounted `setText` or `setBox` updates synchronously so a numeric label and its marks can agree in the same `paint()`; unchanged inputs do no work. Font/language changes retain the text element and surrounding actors; line children are updated only when layout inputs or measured metrics change. Move a containing group when animating the label and its box together, or update `setBox` when the intended location changes.

## Persistent lines during animation

Repeated `layout()`, `L.reflow(root)` and `L.ready(root)` calls preserve the same settled `tspan` nodes when the source, translation, box, padding, alignment, line spacing and computed font metrics are unchanged. Moving a parent group or changing paint colors does not recreate the text. This prevents avoidable SVG glyph repaint churn during camera motion.

The layout cache also includes font-loading state and a font-face load epoch: loading a real face can replace fallback glyphs without changing the CSS family name. Every reuse checks the current local glyph bounds, so changed measured metrics trigger wrapping and alignment again. A detached element, unavailable geometry, negative/non-finite measurements or zero glyph dimensions for nonempty text is **unmeasured** and cannot be cached as a successful fit. Reattaching the actor and calling `L.ready` measures it again. Empty text remains a valid empty label.

This is an internal optimization, not a public request to skip measurement. Use `setText` and `setBox` for edits; the actor owns its line children. Explicit overflow remains visible and auditable. Disposal removes the actor from later reflows and font/language callbacks.
...
Reproduce the layout lifecycle checks with `node --test tests/layout.cjs tests/layout-persistence.cjs` in the kit. The persistence suite also contains a native-browser check of RU/EN × sans/serif and a real font face replacing fallback glyphs. Run it with `VLK_NATIVE_BROWSER=1 node --test tests/layout-persistence.cjs` when Playwright and its browser are already installed. Optional `VLK_PLAYWRIGHT_MODULE` and `VLK_BROWSER_EXECUTABLE` select existing installations; the check downloads nothing. It checks node identity and native glyph geometry, not a full film or pixel-level typography.
