...
- Control bar: a captions button left of `···` (also T / Е) hides the slide's title, caption and HTML text boxes with a 0.35 s fade and brings them back, so the drawing with its own designations (5′/3′, numbers, object labels) can be looked at alone; the state is not persisted.
